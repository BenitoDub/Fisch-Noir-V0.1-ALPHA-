...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Fisch Noir V0.1 | %s fps | %s ms'):format(
        math.floor(FPS),
...
local Tabs = {
    ...
    LocalPlayer = Window:AddTab('Visuals'),
...
local AutoShakeGroup = Tabs.Main:AddLeftGroupbox('Shake The rod')
local AutoReelGroup = Tabs.Main:AddLeftGroupbox('Reel the rod')
local AutoCastGroup = Tabs.Main:AddLeftGroupbox('Cast the rod')
local FishUtilitiesGroup = Tabs.Main:AddRightGroupbox('Fish options')
--local EventGroup = Tabs.Main:AddRightGroupbox('Event')
local ZoneCastGroup = Tabs.Main:AddRightGroupbox('cast in zone')
...
CreditsGroup:AddLabel('Made by benitoDub')
CreditsGroup:AddLabel('If you see this then why the fuck r u hacking?')

local DiscordButton = CreditsGroup:AddButton({
    Text = 'Copy Youtube Channel',
    Func = function()
        setclipboard('https://www.youtube.com/@BenitoDub')
    end,
    DoubleClick = false,
    Tooltip = 'Sub to my yt'
})

local DiscordButton2 = CreditsGroup:AddButton({
    Text = 'Copy Github link',
    Func = function()
        setclipboard('https://github.com/BenitoDub')
    end,
    DoubleClick = false,
    Tooltip = 'All my scripts!'
})

CreditsGroup:AddLabel('HEHE FUCK YOU')
